/* IPP – Ados only (12–25), caps vécus + PDF cover with big logo */
...
document.querySelectorAll(".cta button").forEach(btn=>btn.addEventListener("click", async()=>{
  await loadQuestions(); start();
}));

async function loadQuestions(){
  const res = await fetch("data/questions_ados.json"); QUESTIONS = await res.json();
...
function renderSynthese(perc,ADN,CAP,capsVecus){
  ...
  el("#synthese").innerHTML=`<h3>Synthèse chiffrée</h3><table><thead><tr><th>Profil</th><th>Énergie</th><th>Repère</th></tr></thead><tbody>${rows}</tbody></table>`;
}
...
async function buildPremiumPdfDoc(ADN,CAP,perc,R){
  ...
  // Append the visible report section as pages
